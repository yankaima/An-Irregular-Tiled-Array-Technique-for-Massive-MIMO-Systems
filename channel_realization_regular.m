function [H,H4,Fopt,Wopt]=channel_realization(Ns,Nt,Nr,realization,angle_sigma,AoD_m_thetamin,AoD_mthetamax,AoA_m_thetamin,AoA_mthetamax,patternrE,Zr,Zt_64,Zt_16)

H=zeros(Nr,Nt,realization);
H4=zeros(Nr/4,Nt,realization);
...
% H(:,:,reali) = scatteringchanmtx_1(newtxpos,rxpos,AoD,AoA,g,vert);
    ...
    rxarray2 =phased.URA([sqrt(Nr)/2 sqrt(Nr)/2],lambda/2);
rxpos2 = getElementPosition(rxarray2)/lambda;
rxpos2=[rxpos2(2,:);rxpos2(3,:);rxpos2(1,:)];

        H4(:,:,reali) = scatteringchanmtx_0(txpos,rxpos2,AoD,AoA,g,patternrE);
    
    
    H4(:,:,reali)=Zt_16*H4(:,:,reali)*Zr;
